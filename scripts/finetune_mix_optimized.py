...
Run from project root:
    python scripts/finetune_mix_enhanced.py
"""
...
logger = get_logger(
    __name__,
    log_file=str(PROJECT_ROOT / "experiments/logs/finetune_mix_enhanced.log"),
)
...
class MixOnlyDataset(Dataset):
    ...
    def __init__(
        self,
        dataframe: pd.DataFrame, # Accept a DataFrame directly for consistency
        mel_path: str | Path,
...
def main() -> None:
    ...
    # Use a dedicated directory for this enhanced fine-tuning run
    out_ckpt_dir = PROJECT_ROOT / "checkpoints_finetune_mix_enhanced"
    ...
    # --- ENHANCEMENT 1: Freeze early layers (optional, experiment) ---
    # Consider freezing the initial conv layers if they capture generic features
    # for i, (name, param) in enumerate(model.named_parameters()):
    #     if i < 10: # Freeze first N layers (adjust N based on your model)
    ...
    # --- ENHANCEMENT 2: Separate LR for different parts (optional, experiment) ---
    # base_params = []
    # top_params = []
    # for name, param in model.named_parameters():
    #     if "classifier" in name or "attention" in name: # Adjust names based on your model
    #         top_params.append(param)
    #     else:
    #         base_params.append(param)
    # params = [
    #     {'params': base_params, 'lr': 1e-5}, # Lower LR for base
    #     {'params': top_params, 'lr': 1e-3}  # Higher LR for classifier/attention
    # ]
    # optimizer = torch.optim.AdamW(params, weight_decay=1e-4)

    # --- Standard Optimizer for now ---
    # ── Fine-tuning config (Enhanced) ────────────────────────────────────────────
    FINETUNE_EPOCHS = 100 # Maybe shorter if using a warmup/cosine schedule
    FINETUNE_LR     = 1e-4 # Starting LR for the schedule
    PATIENCE        = 10 # Adjust patience

    # --- ENHANCEMENT 3: Different Optimizer (AdamW is good, maybe try Adam) ---
    optimizer = torch.optim.AdamW(model.parameters(), lr=FINETUNE_LR, weight_decay=1e-4)
    # optimizer = torch.optim.Adam(model.parameters(), lr=FINETUNE_LR, weight_decay=1e-4) # Alternative

    # --- ENHANCEMENT 4: Sophisticated Scheduler ---
    # Use CosineAnnealingWarmRestarts or a OneCycleLR if available
    # For now, CosineAnnealingLR with warmup
    from torch.optim.lr_scheduler import CosineAnnealingWarmRestarts, LambdaLR
    
    # Simple linear warmup scheduler helper
    ...
    # Apply warmup first, then cosine annealing
    warmup_scheduler = LambdaLR(optimizer, lr_lambda=warmup_lambda)
    main_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=FINETUNE_EPOCHS - warmup_lambda(0)*FINETUNE_EPOCHS, # Adjust T_max after warmup
        eta_min=1e-6
    )
    # Combine schedulers manually in the loop
    def step_schedulers(epoch):
        if epoch <= 5: # Warmup period
            warmup_scheduler.step()
        else: # After warmup, use main scheduler
            main_scheduler.step()
            
    # Alternative: CosineAnnealingWarmRestarts (can be powerful)
    ...
    console.rule("[bold cyan]Fine-tuning Enhanced (ALL data) — mix-only")
    console.print(f"Epochs: {FINETUNE_EPOCHS}  |  Initial LR: {FINETUNE_LR}  |  Patience: {PATIENCE}")
    ...
    for epoch in range(1, FINETUNE_EPOCHS + 1):
        ...
        # Step scheduler (combined warmup + cosine)
        step_schedulers(epoch)
        # If using CosineAnnealingWarmRestarts:
        # scheduler.step(epoch) # Pass epoch if needed
    ...
    console.print(f"\n[bold green]Enhanced Fine-tuning complete![/bold green]")
    console.print(f"Best checkpoint → [cyan]{best_path}[/cyan]")
    console.print(
        "\n[yellow]Update inference_config.yaml:[/yellow]\n"
        f"  model_checkpoint: checkpoints_finetune_mix_enhanced/best_multibranch_model.pth"
    )
