...
from sklearn.model_selection import train_test_split # Import here too
...
def main() -> None:
    ...
    # ── Fine-tuning config ────────────────────────────────────────────────────
    # Increase epochs significantly since we now
    # Adjust patience relative to the new total epochs if needed
    FINETUNE_EPOCHS = 100 # Increased epochs
    FINETUNE_LR     = 1e-4
    PATIENCE        = 10 # Increased patience, maybe make it ~10% of epochs?
    ...
    console.rule("[bold cyan]Fine-tuning — mix-only (using ALL data)")
